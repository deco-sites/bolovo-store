...
export default function ButtonsPagination({ props, page }: { props?: ButtonsPaginationProps, page: ProductListingPage }) {

    const { labelNext, layoutPagination, labelPrevious, labelButtonTop } = { ...DEFAULT_PROPS, ...props }
    const { pageInfo } = page
    const numberMaxPage: number = Math.ceil((pageInfo.records ?? 0) / (pageInfo.recordPerPage ?? 0))
    const arrayNumber: Array<number> = []

    const hrefNavigation = pageInfo.nextPage?.slice(0, -1) ?? pageInfo.previousPage?.slice(0, -1)
                            ...
                            <>
                                {
                                    pageInfo.currentPage === number ?
                                        <LinkNumberPage number={number} currentPage={pageInfo.currentPage} href={hrefNavigation} /> : number === 5 && pageInfo.currentPage > 5 ?
                                            <span>. . .</span>
                                            : number > 4 && number < arrayNumber.length - 2 ?
                                                null :
                                                number < arrayNumber.length - 1 && number > 4 && pageInfo.currentPage < arrayNumber.length - 1 ?
                                                    <span>. . .</span> :
                                                    <LinkNumberPage number={number} currentPage={pageInfo.currentPage} href={hrefNavigation} />}
                            </>
                        )
                        )}
                    </div>
