...
export interface Props {
    ...
    */
    ...
    /**
     * @title Link of video 
     * @description Infos of secord column
     */
    videoLink: string;

}

export default function TextPlusVideo({ directionInverse, image, textContent, listImages, videoLink }: Props) {
    return (
        <div class="flex flex-row px-2 w-full py-8 2xl:py-16">
            <div style={{ flexDirection: directionInverse ? "row-reverse" : "row" }} class="flex flex-row justify-center items-center gap-4 w-full">
                <div class="flex flex-col justify-between h-full items-center w-2/4 gap-3 2xl:gap-20">
                    <Image src={image.src} alt={image.alt} width={350} height={69} loading={"eager"} class="max-w-[200px] md:max-w-[220px] xl:max-w-[350px] 2xl:max-w-[400px]" />
                    <div dangerouslySetInnerHTML={{ __html: textContent }} class=" font-AcuminPro-Regular text-[10px] leading-4 lg:text-[13px] lg:leading-5 xl:text-sm xl:leading-6 2xl:text-base 2xl:leading-7">
                    ...
                    <div class="flex flex-row gap-2">
                        {listImages.map((img) => {
                            return (
                                <div>
                                    <Image src={img.src} alt={img.alt} width={100} height={37} loading={"eager"} class="max-w-[70px] xl:max-w-[100px] 2xl:max-w-[150px]" />
                                </div>
                            )
                        })}
                    </div>
